...
from google.cloud.dataproc_v1beta2.services.autoscaling_policy_service.transports.base import _API_CORE_VERSION
from google.cloud.dataproc_v1beta2.services.autoscaling_policy_service.transports.base import _GOOGLE_AUTH_VERSION
...
def test_create_autoscaling_policy(transport: str = 'grpc', request_type=autoscaling_policies.CreateAutoscalingPolicyRequest):
    ...
    with mock.patch.object(
        ...
        call.return_value = autoscaling_policies.AutoscalingPolicy(
            id='id_value',
            name='name_value',
            basic_algorithm=autoscaling_policies.BasicAutoscalingAlgorithm(yarn_config=autoscaling_policies.BasicYarnAutoscalingConfig(graceful_decommission_timeout=duration_pb2.Duration(seconds=751))),
        )
...
def test_autoscaling_policy_service_base_transport_with_credentials_file():
    ...
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.dataproc_v1beta2.services.autoscaling_policy_service.transports.AutoscalingPolicyServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_autoscaling_policy_service_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.cloud.dataproc_v1beta2.services.autoscaling_policy_service.transports.AutoscalingPolicyServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.AutoscalingPolicyServiceTransport()
        adc.assert_called_once()
...
def test_autoscaling_policy_service_transport_create_channel_old_api_core(transport_class, grpc_helpers):
    # If credentials and host are not provided, the transport class should use
    # ADC credentials.
    with mock.patch.object(google.auth, "default", autospec=True) as adc, mock.patch.object(
        grpc_helpers, "create_channel", autospec=True
...
@requires_api_core_lt_1_26_0
def test_autoscaling_policy_service_transport_create_channel_user_scopes(transport_class, grpc_helpers):
    # If credentials and host are not provided, the transport class should use
    # ADC credentials.
    with mock.patch.object(google.auth, "default", autospec=True) as adc, mock.patch.object(
        grpc_helpers, "create_channel", autospec=True
